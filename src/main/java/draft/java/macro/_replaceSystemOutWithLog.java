...
public class _replaceSystemOutWithLog implements _macro<_type> {
    ...
    public _replaceSystemOutWithLog(
        ...
        if( !vars.contains("name")){
            throw new DraftException("MISSING $var: \"name\" in : "+ loggerStatementsFormat +System.lineSeparator() +
                    "... used for representing the field NAME of the Logger");
        }
        if( !vars.contains("any")){
            throw new DraftException("MISSING $var: \"any\" in : "+ loggerStatementsFormat +System.lineSeparator() +
                    "... used for representing the content to be logged");
        }
        if( !adHocLogger.list$().contains("className")){
            throw new DraftException("MISSING $var: \"className\" in : "+ adHocLogger +System.lineSeparator() +
    ...
    public _type apply(_type _t) {
        if( $anySystemOut.listIn(_t).size() > 0 ) {
            ...
            $anySystemOut.replaceAllIn(_t, $snip.of( loggerStatementsFormat )
