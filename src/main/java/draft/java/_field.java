...
/**
 * Model of a Java Field
 * (A Facade linked to a {@link VariableDeclarator} and its parent {@link FieldDeclaration} )
 *
 * NOTE: many variables can be declared in a single {@link FieldDeclaration}:
 * <PRE>
 * int x,y,z;
 * </PRE>
 * each of the above will be (3) separate {@link _field} (s)
 * each with the same {@link FieldDeclaration} root,
 * and with (3) separate {@link VariableDeclarator}
 *
 * thus, each {@link _field} is a reference to the {@link VariableDeclarator} and it can "lookup"
 * it's parent {@link FieldDeclaration}
 *
 */
public final class _field
     ...
     *
     * FieldDeclaration fd = Ast.field("int x,y,z;");
     * List<_field> _fs = _fields.of(fd);
     * //_fs is a list of (3) _fields
     *
    ...
    /**
     * Gets the (effective modifiers) which is the union of the explicit modifiers
     * (returned from {@link #getModifiers()} and the implied modifiers, that are
