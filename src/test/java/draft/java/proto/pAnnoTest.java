...
public class pAnnoTest extends TestCase {
 
    
    public void testAny(){
        _class _c = _class.of("C");
        assertEquals( 0, pAnno.list(_c).size());
        
        //add a top level annotation
        _c.annotate(Deprecated.class);
        assertEquals( 1, pAnno.list(_c).size());
        
        pAnno.forEach(_c, a-> System.out.println(a.getName()));
        pAnno.forEach(_c, a-> !a.hasValues(), a-> System.out.println( a) );
    }
    ...
    public void testMatchClass(){
        ...
        assertTrue(pAnno.of(R.class).matches("R"));
        assertTrue(pAnno.of(R.class).matches("@draft.java.proto.pAnnoTest.R"));
        
        assertTrue(pAnno.of(R.class).matches("R()"));
        assertTrue(pAnno.of(R.class).matches("@draft.java.proto.pAnnoTest.R()"));
        
        assertTrue(pAnno.of(R.class).matches("R(1)"));
        assertTrue(pAnno.of(R.class).matches("@draft.java.proto.pAnnoTest.R(2)"));        
        
        assertTrue(pAnno.of(R.class).matches("R(value=1)"));
        assertTrue(pAnno.of(R.class).matches("@draft.java.proto.pAnnoTest.R(value=2)"));   
        
        assertTrue(pAnno.of(R.class, "()").matches("@draft.java.proto.pAnnoTest.R()"));   
        assertTrue(pAnno.of(R.class, "()").matches("R()"));   
        
        assertTrue(pAnno.of(R.class, "($any$)").matches("@draft.java.proto.pAnnoTest.R()"));   
        assertTrue(pAnno.of(R.class, "($any$)").matches("R()"));   
        
        assertTrue(pAnno.of(R.class, "($any$)").matches("@draft.java.proto.pAnnoTest.R(1)"));   
        assertTrue(pAnno.of(R.class, "($any$)").matches("R(2)"));
        
        assertTrue(pAnno.of(R.class, "($any$)").select("@draft.java.proto.pAnnoTest.R(1)").is("any", Expr.of(1) ) );   
        assertTrue(pAnno.of(R.class, "($any$)").select("@draft.java.proto.pAnnoTest.R(1)").is("any", "1" ) );  
        ...
        assertTrue(pAnno.of(S.class, "($any$)").matches("S()"));
        assertTrue(pAnno.of(S.class, "($any$)").matches("S(Float.class)"));
        assertTrue(pAnno.of(S.class, "($any$)", a-> a.hasValue(v -> v.isClassExpr())).matches("@S(Float.class)"));
        assertFalse(pAnno.of(S.class, "($any$)", a-> a.hasValue(v -> v.isClassExpr())).matches("@S"));
        assertFalse(pAnno.of(S.class, "($any$)", a-> a.hasValue(v -> v.isClassExpr())).matches("@S({Float.class, String.class})"));
    ...
    public void testFullyQualified(){        
        ...
        // 
        pAnno $a = pAnno.of(R.class);
        
        @draft.java.proto.pAnnoTest.R
        class C{}        
    ...
    public void testFirst(){
        ...
        _class _c = _class.of(C.class);
        assertNotNull( pAnno.first(_c, "name($any$)") );
        ...
        assertNotNull( pAnno.first(_c, "name($any$)", (a) -> a.hasValue(3)) );
        assertNotNull( pAnno.first(_c, "name(3)"));        
        assertNotNull( pAnno.first(_c, "name(3)", _a-> _a.hasValue(3)) );
    }
     
    public void testStatic$anno(){
        pAnno $a = pAnno.of("@name");
        ...
        _c = $a.replaceIn(_c, pAnno.of("@name2"));
        assertEquals( 0, $a.selectListIn(_c).size() ); //verify they are all changed
        assertEquals( 3, pAnno.of("@name2").selectListIn(_c).size() ); //verify they are all changed
    }

    public void testDynamicAnno(){
        //any @NAME annotation with a prefix
        pAnno $a = pAnno.of("@name(prefix=$any$)");
        ...
        $a.replaceIn(_c, pAnno.of("@name2(string=$any$)") );
