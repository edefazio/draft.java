...
public class _autoHashCodeTest extends TestCase {
    ...
    public void testR(){
        ...
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "ss") ).findAny().isPresent()); //short
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "c") ).findAny().isPresent()); //char
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "Float.floatToIntBits(f)") ).findAny().isPresent()); //float
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "(int) (Double.doubleToLongBits(d) ^ (Double.doubleToLongBits(d) >>> 32))")).findAny().isPresent());
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "(int) (l ^ (l >>> 32))")).findAny().isPresent());

        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "java.util.Arrays.hashCode(bys)")).findAny().isPresent());
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "java.util.Objects.hashCode(str)")).findAny().isPresent());
        assertTrue( ss.stream().filter(s-> s.tokens.has("fieldHash", "java.util.Arrays.deepHashCode(uuids)")).findAny().isPresent());
    }
